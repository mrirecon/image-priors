#!/bin/bash
# compare prior that are trained on different amounts of images from the same dataset.
set -e 
...
GRAPH2=$ROOT_PATH/logs/exported/pixelcnn_abide_1000
GRAPH3=$ROOT_PATH/logs/exported/pixelcnn_abide_300

DATA_PATH=$ROOT_PATH/data/kspaces/mprage

mkdir -p $ROOT_PATH/results/trn_size/$PATTERN
cd $ROOT_PATH/results/trn_size/$PATTERN
...
bart pics -g -i100 -d4 -R TF:{$GRAPH2}:0.8 und_kspace coilsen prior_abide_1000_pics
bart pics -g -i100 -d4 -R TF:{$GRAPH3}:0.8 und_kspace coilsen prior_abide_300_pics
...
bart nlinv -g -d4 -a660 -b44 -i14 -C50 --reg-iter=5 -R LP:{$GRAPH2}:0.8:1 und_kspace prior_abide_1000_nlinv prior_abide_1000_nlinv_coils
bart nlinv -g -d4 -a660 -b44 -i14 -C50 --reg-iter=5 -R LP:{$GRAPH3}:0.8:1 und_kspace prior_abide_300_nlinv prior_abide_300_nlinv_coils